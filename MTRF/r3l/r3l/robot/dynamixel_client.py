...
class DynamixelClient:
    ...
    def write_desired_cur(self, motor_ids: Sequence[int],
        ...
        NOTE: The Dynamixel must be set to current control before using this.
        Args:
            motor_ids: The motor IDs to write to.
            currs: Currents in mA to write.
        """
        assert len(motor_ids) == len(currs)
        # Convert to Dynamixel current scale.
        currs = currs / DEFAULT_CUR_SCALE
        self.sync_write(motor_ids, currs, ADDR_GOAL_CURRENT, LEN_GOAL_CUR)

    def write_desired_vel(self, motor_ids: Sequence[int],
                          vels: np.ndarray):
        """Writes the given desired velocities.
        NOTE: The Dynamixel must be set to velocity control before using this.
        Args:
            motor_ids: The motor IDs to write to.
            vels: Velocities to write.
        """
        assert len(motor_ids) == len(vels)
        # Convert to Dynamixel current scale.
        vels = vels / DEFAULT_VEL_SCALE
        self.sync_write(motor_ids, vels, ADDR_GOAL_VELOCITY, LEN_GOAL_VELOCITY)
