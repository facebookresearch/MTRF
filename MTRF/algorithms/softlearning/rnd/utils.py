...
def get_rnd_networks_from_variant(variant, env):
    ...
    if rnd_params.get("kwargs", {}).get("observation_keys", None) is not None:
        observation_keys = rnd_params["kwargs"]["observation_keys"]
    else:
    ...
    predictor_network.observation_keys = observation_keys
    target_network.observation_keys = observation_keys
