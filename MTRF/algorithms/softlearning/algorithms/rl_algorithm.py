...
class RLAlgorithm(Checkpointable):
    ...
    def __init__(
        ...
        if sampler:
            ...
            if hasattr(self.sampler, 'set_algorithm'):
                self.sampler.set_algorithm(self)
    ...
    def _training_paths(self):
        ...
        if (self._save_training_video_frequency > 0
            ...
            for i, path in enumerate(reversed(paths)):
                video_frames.extend(path.pop('images', []))
            video_file_name = f'training_path_epoch{self._epoch}.mp4'
            ...
            try:
                ...
                print(f"Cannot save video with {len(video_frames)} frames")
    ...
    def _evaluation_paths(self, policy, evaluation_env):
        ...
        with policy.set_deterministic(self._eval_deterministic):
            paths = rollouts(
                ...
                sampler_class=self.sampler.__class__,
                algorithm=self,
                extra_fields=self.sampler.pool.fields,
